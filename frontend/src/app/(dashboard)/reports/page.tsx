  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <button 
          onClick={() => alert('Opening report creation wizard...')}
          className="neo-raised bg-[#e8eaf0] px-6 py-3 rounded-xl flex items-center gap-2 text-indigo-600 font-semibold hover:neo-inset transition-all active:scale-95"
        >
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-10">
        ...
        <div className="flex items-end">
          <button 
            ...
            className="neo-raised bg-[#e8eaf0] w-full py-3 rounded-xl text-indigo-600 font-semibold hover:neo-inset flex items-center justify-center gap-2 transition-all active:scale-95"
          >
  ...
  return (
    <div className={`${inset ? 'neo-inset' : 'neo-raised hover:-translate-y-1'} bg-[#e8eaf0] rounded-xl p-6 group transition-transform`}>
      ...
      <div className="grid grid-cols-2 gap-3">
        {(r.status === 'completed' || r.title === 'Scholarship Eligibility') && (
          <>
            <button 
              onClick={() => window.open(`/report-preview/${r.id}`, '_blank')} 
              className="neo-raised bg-[#e8eaf0] py-2 rounded-lg text-xs font-semibold text-indigo-600 hover:neo-inset transition-all"
            >
              Preview
            </button>
            <button 
              onClick={() => {
                window.open(`/report-preview/${r.id}?pdf=true`, '_blank');
              }} 
              className="neo-raised bg-[#e8eaf0] py-2 rounded-lg text-xs font-semibold text-indigo-700 flex items-center justify-center gap-1 hover:neo-inset transition-all"
            >
        ...
        {r.status === 'processing' && (
          <>
            ...
            <button 
              onClick={() => alert(`You will be notified when ${r.title} is ready.`)} 
              className="neo-raised bg-[#e8eaf0] py-2 rounded-lg text-xs font-semibold text-indigo-700 flex items-center justify-center gap-1 hover:neo-inset transition-all"
            >
        ...
        {r.status === 'archived' && (
          <button 
            onClick={() => alert(`Restoring ${r.title} to active reports...`)} 
            className="col-span-2 neo-raised bg-[#e8eaf0] py-2 rounded-lg text-xs font-semibold text-slate-600 flex items-center justify-center gap-1 hover:neo-inset transition-all"
          >
            <span className="material-symbols-outlined text-sm">unarchive</span>Restore to Active
          </button>
        )}
        {r.status === 'draft' && r.title !== 'Scholarship Eligibility' && (
          <button 
            onClick={() => alert(`Continuing edit for ${r.title}...`)} 
            className="col-span-2 neo-raised bg-[#e8eaf0] py-2 rounded-lg text-xs font-semibold text-indigo-600 hover:neo-inset transition-all"
          >
            Continue Editing
          </button>
        )}
